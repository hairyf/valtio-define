...
): [S[K], Dispatch<SetStateAction<S[K]>>] {
...
export function storeToStates<S extends object>(
  store: Store<S>,
  options?: { sync?: boolean },
): {
  [K in keyof S]: [S[K], Dispatch<SetStateAction<S[K]>>]
} {
  return Object.fromEntries(Object.keys(store.$state).map(key => [key, storeToState(store, key as keyof S, options)])) as any
}
